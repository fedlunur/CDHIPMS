...
export default function ActivityBoardList({}) {
  ...
  const { allactivities, alltasks, activities_by_project } = DataService();
  ...
  const addActivity = async () => {
    if (activityInputValue.trim() !== "") {
      try {
        const response = await api.post("/activitylist/", {
  ...
  return (
    <Layout>
      <div className="w-[100%] flex flex-col space-y-4" style={{ height: "90vh" }}>
